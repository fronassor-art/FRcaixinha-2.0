...
class _OperationsDashboardScreenState extends State<OperationsDashboardScreen> {
  ...
  Future<void> load() async {
    ...
    try {
      data = await api.get('/admin/operations/dashboard');
      error = null;
    } catch (e) {
      error = e.toString();
    } finally {
  ...
  Widget build(BuildContext context) {
    final d = data;
    final rec = d?['reconciliation'];
    return Scaffold(
      appBar: AppBar(title: const Text('Operação e Reconciliação')),
            ...
            if (d != null) ...[
              Card(
                child: ListTile(
                  ...
                  title: Text('Status: ${d['go_no_go']}'),
                  subtitle: Text('Reconciliação: ${rec['status']}'),
                ),
              ),
              ...
              ...((rec['findings'] as List).map(
                (x) => ListTile(
                  dense: true,
                  leading: Icon(
                    x['status'] == 'PASS' ? Icons.check : Icons.error,
                  ),
                  title: Text(x['code']),
                  subtitle: Text(x['details']),
                ),
              )),
            ],
          ],
  ...
  Widget _tile(String a, String b) => Card(
    child: ListTile(
      title: Text(a),
