...
class _AdminLoansScreenState extends State<AdminLoansScreen>{
  ...
  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('Solicitações de empréstimo')),body:RefreshIndicator(onRefresh:load,child:ListView(padding:const EdgeInsets.all(12),children:[if(error!=null)Text(error!),...items.map((x)=>Card(child:ListTile(title:Text(x['member_name']??'Membro'),subtitle:Text('R\$ ${x['principal']} • ${x['installments']} parcelas • taxa ${x['monthly_rate']}'),trailing:Wrap(spacing:4,children:[IconButton(tooltip:'Rejeitar',onPressed:()=>decide(x['id'],false),icon:const Icon(Icons.close)),IconButton(tooltip:'Aprovar',onPressed:()=>decide(x['id'],true),icon:const Icon(Icons.check))]))))])));
}
