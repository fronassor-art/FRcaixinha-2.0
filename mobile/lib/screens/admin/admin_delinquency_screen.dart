...
class _AdminDelinquencyScreenState extends State<AdminDelinquencyScreen> {
  ...
  Future<List<AdminDelinquencyItem>> _loadItems() =>
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<FinancialObligationType?>(
                          ...
                          items: const [
                            ...
                            DropdownMenuItem(
                              key: Key('delinquency-filter-type-agreement'),
                              value:
                                  FinancialObligationType.agreementInstallment,
                              child: Text('Parcela de acordo'),
                            ),
                          ],
                          onChanged:
                              (value) =>
                                  setSheetState(() => obligationType = value),
                        ),
                        ...
                        DropdownButtonFormField<FinancialObligationStatus?>(
                          ...
                          onChanged:
                              (value) => setSheetState(() => status = value),
                        ),
                        SwitchListTile(
                          ...
                          onChanged:
                              (value) =>
                                  setSheetState(() => overdueOnly = value),
                        ),
                        OutlinedButton(
                          key: const Key('delinquency-filter-due-from'),
                          onPressed: () => selectDate(true),
                          child: Text(
                            'Data inicial de vencimento: ${_date(dueFrom)}',
                          ),
                        ),
                        ...
                        OutlinedButton(
                          ...
                          child: Text(
                            'Data final de vencimento: ${_date(dueTo)}',
                          ),
                        ),
                        const SizedBox(height: 16),
                        Row(
  ...
  String _type(AdminDelinquencyItem item) => switch (item.obligationType) {
    FinancialObligationType.contribution => 'Contribuição',
    FinancialObligationType.loanInstallment => 'Parcela do empréstimo',
    FinancialObligationType.agreementInstallment => 'Parcela de acordo',
    FinancialObligationType.unknown => 'Tipo de obrigação não identificado',
  };
      ...
      child: Wrap(
        ...
        children: [
          _metric(
            ...
            '${summary.delinquentMembersCount}',
          ),
  ...
  Widget _itemCard(AdminDelinquencyItem item) {
    ...
    final identification = switch (item.obligationType) {
      FinancialObligationType.contribution =>
        'Competência: ${item.competence ?? 'Não informada'}',
      FinancialObligationType.loanInstallment =>
        'Empréstimo #${item.loanId ?? '-'} • Parcela ${item.installmentNumber ?? '-'}',
      FinancialObligationType.agreementInstallment =>
        'Acordo do empréstimo #${item.loanId ?? '-'} • Parcela ${item.installmentNumber ?? '-'}',
      FinancialObligationType.unknown => 'Obrigação não identificada',
    };
    final lines = <String>[
      identification,
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              item.memberName ?? 'Participante #${item.memberId}',
                  ...
                  children: [
                    ...
                    if (items.isEmpty)
                      const Padding(
                        ...
                        child: Center(
                          child: Text('Nenhuma obrigação em aberto.'),
                        ),
                      )
                    else
                      ...items.map(_itemCard),
