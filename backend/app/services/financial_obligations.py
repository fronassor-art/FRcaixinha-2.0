...
def installment_item(db,i,as_of=None):
 ...
 return {'member_id':loan.member_id,'obligation_type':'LOAN_INSTALLMENT','obligation_id':i.id,'competence':None,'loan_id':i.loan_id,'installment_number':i.number,'due_date':i.due_date.isoformat(),'amount_due':str(money(i.amount)+money(i.penalty_amount)),'amount_paid':str(paid),'outstanding_amount':str(out),'financial_status':status,'days_overdue':max(0,(as_of.date()-i.due_date).days) if status=='OVERDUE' else 0,'penalty_outstanding':str(max(ZERO,money(i.penalty_amount)-money(i.paid_penalty_amount))),'interest_outstanding':str(max(ZERO,money(i.interest)-min(money(i.interest),money(i.paid_amount)))),'payment_id':p.id if p else None,'receipt_available':_receipt(db,p)}
